...
namespace SonikLibFileSystemsControllers
{
	...
	public:
		...
		bool ReadBinary(char* buffer, unsigned long size);

		//(テキストモード専用)指定した文字数分TEXTを読み込みます。
		virtual bool ReadText_CHR(char* buffer, uint32_t ReadSize) =0;
		//(テキストモード専用)指定された行数分TEXTを読み込みます。(改行は削除されます。)
		virtual bool ReadText_Line(SonikLib::SonikString& LineStr, unsigned long GetRowCnt =1) =0;
		//指定された行数分TEXTを読み込みます。改行は削除され、改行で分割されたQueueとして取得します。
		virtual bool ReadText_LineQueue(SonikQueue::SonikAtomicQueue<SonikLib::SonikString>& LineStrQueue, unsigned long GetRowCnt =1) =0;
