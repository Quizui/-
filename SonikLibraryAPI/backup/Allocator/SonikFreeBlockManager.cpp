...
{
	//カテゴリ算出
	void GetCategory(unsigned long Val, unsigned long& First, unsigned long& Second, SonikTLSFSecondCategoryBitDepth SetDepth)
	{
		First = Val;
		Second = Val;
...
namespace SonikBTMAFreeBlock
{
	...
	//イニシャライザ
	bool FreeBlockManager::Initialize(unsigned long SetSize, SonikTLSFSecondCategoryBitDepth SetSecondBitDepth)
	{
		unsigned long Bit = SonikMathBit::GetMSBFor32bit(SetSize);
		...
		for(unsigned long looped_ =0; looped_ < Bit; ++looped_)
		{
	...
	{
		unsigned long pValFirst =0;
		unsigned long pValSecondary = 0;
	...
	//フリーブロックを取得します。
	SonikBaundaryBlock::SonikBaundaryPre* FreeBlockManager::GetFreeBlock(unsigned long GetSize)
	{
		unsigned long pValFirst = GetSize;
		unsigned long pValSecondary =  GetSize;
		unsigned long msbcount = 0;
	...
	{
		unsigned long pValFirst; 		//ファーストカテゴリ値
		unsigned long pValSecondary;	//セカンダリカテゴリ値
