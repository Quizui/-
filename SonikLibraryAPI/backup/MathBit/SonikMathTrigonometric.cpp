...
namespace SonikMath
{
	...
	SonikLib::SonikAtomicLock global_initblock;
	...
	char __SONIK_MATH_INITIALIZECALLED_GLOBAL_VALIABLE_NUMBER__ = Initialize();
	...
	double Sin(double digree)
	{
		...
		digree *= math_signed;
		digree -= 360.0 * (static_cast<long>( digree / 360.0 ) );
		...
		index = static_cast<long>( digree * 10.0 );
	...
	float radian_To_digree(float radian)
	{
		...
		void SinCosValueOutPut(void)
		{
			...
			while(digree < 360.0)
			{
				...
				sprintf(buffer, "Value:[SinF] = [sin] %-7.5f = %-7.5f   \n", ret, ret_c);
				str += buffer;
				memset(buffer, 0, 300);
				...
				sprintf(buffer, "Value:[CosF] = [cos] %-7.5f = %-7.5f   \n", ret, ret_c);
				str += buffer;
				memset(buffer, 0, 300);
				...
				sprintf(buffer, "Value:[TanF] = [tan] %-7.5f = %-7.5f   \n", ret, ret_c);
				str += buffer;
				memset(buffer, 0, 300);
			...
			sprintf(buffer, "負のSin値　負のCos値 表 ============================\n\n");
			str += buffer;
			memset(buffer, 0, 300);
