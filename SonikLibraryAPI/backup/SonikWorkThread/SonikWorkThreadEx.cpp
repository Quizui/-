...
#include "..\\Function\\WorkerFunctor_Base.hpp"
...
namespace SonikThreadImpl
{
	...
	private:
		...
		//実際にコールする関数オブジェクト
		SonikLib::SonikAllocSmtPtr<SonikThreadPack::ThreadPack> Access_; //受け取り口
		SonikThreadPack::ThreadPack FuncObj;

		//関数パックのキューオブジェクトへのポインタ
		SonikQueue::SonikAtomicQueue<SonikLib::SonikAllocSmtPtr<SonikThreadPack::ThreadPack>>* FuncQueue_;
	...
	public:
		...
		bool SetCallFunction(SonikLib::SonikAllocSmtPtr<SonikLib::FunctionSystem_Base>& CallFunctionObject, bool Deleted = false);

		//静的関数内で使用
		SonikThreadPack::ThreadPack& GetFunctionPointer(void);
		std::condition_variable& GetConditionVariable(void);
		SonikLib::SonikAtomicLock& GetCASLockObject(void);
		unsigned long& GetThreadFlag(void);
		//キューのあんせっと
		...
		//キューポインタをセットします。
		void SetFunctionQueue(SonikQueue::SonikAtomicQueue<SonikLib::SonikAllocSmtPtr<SonikThreadPack::ThreadPack>>* pSetQueue);
		//キューポインタをアンセットします。
	...
	{
		unsigned long& RefFlag = reinterpret_cast<SonikThreadImpl::pImplEx*>(ClassObject)->GetThreadFlag();
		std::condition_variable& RefCond = reinterpret_cast<SonikThreadImpl::pImplEx*>(ClassObject)->GetConditionVariable();
		SonikLib::SonikAtomicLock& RefLock = reinterpret_cast<SonikThreadImpl::pImplEx*>(ClassObject)->GetCASLockObject();
		std::mutex localmtx;
		...
		while(1)
		{
			if(RefFuncObj.pFunc == 0 && RefFuncObj.SmtPtrFunc == 0)
			{
				{
					...
					RefCond.wait(lock,
								[&RefFuncObj, &ClassObject, &RefFlag]{
																		...
																		( RefFuncObj.pFunc == 0 && RefFuncObj.SmtPtrFunc == 0 ) ? Geted_ = false: Geted_ = true;
			...
			if((RefFlag & 0x02) != 0)
			{
				...
				if(RefFuncObj.SmtPtrFunc != 0)
				{
					RefFuncObj.SmtPtrFunc.~SonikAllocSmtPtr();
					RefFlag &= (~0x02);
				};
			};
	...
	SonikThreadImpl::pImplEx::~pImplEx(void)
	{
		...
		if( (ThreadFlag & 0x02) != 0 && (FuncObj.SmtPtrFunc != 0))
		{
			FuncObj.SmtPtrFunc.~SonikAllocSmtPtr();
		};
	...
	bool pImplEx::SetCallFunction(SonikLib::SonikAllocSmtPtr<SonikLib::FunctionSystem_Base>& CallFunctionObject, bool Deleted)
	{
		if( !atmlock_.TryLock() )
		{
			return false;
		...
		//セット
		FuncObj.SmtPtrFunc = static_cast<SonikLib::SonikAllocSmtPtr<SonikLib::FunctionSystem_Base>&&>(CallFunctionObject);
		SetFunctionDeleteFlag(Deleted);
		cond_.notify_one();
		//ミューテックスのアンロックは静的関数内で行う。
		return true;
	...
	std::condition_variable& pImplEx::GetConditionVariable(void)
	{
		return cond_;
	};

	SonikLib::SonikAtomicLock& pImplEx::GetCASLockObject(void)
	{
		return atmlock_;
	};

	unsigned long& pImplEx::GetThreadFlag(void)
	{
		return ThreadFlag;
	};
	...
	void pImplEx::UpdateQueue(void)
	{
		...
		if( Access_ != 0 )
		{
			FuncObj.SmtPtrFunc = std::move(Access_->SmtPtrFunc);

			Access_.~SonikAllocSmtPtr();
	...
	//キューポインタをセットします。
	void pImplEx::SetFunctionQueue(SonikQueue::SonikAtomicQueue<SonikLib::SonikAllocSmtPtr<SonikThreadPack::ThreadPack>>* pSetQueue)
...
namespace SonikThread
{
	...
	bool WorkThreadEx::SetCallFunction(SonikLib::SonikAllocSmtPtr<SonikLib::FunctionSystem_Base> CallFunctionObject, bool DeleteFlag)
	{
		return ImplObject->SetCallFunction(CallFunctionObject, DeleteFlag);
	};
	...
	void WorkThreadEx::Set_ExternalQueue(SonikQueue::SonikAtomicQueue<SonikLib::SonikAllocSmtPtr<SonikThreadPack::ThreadPack>>* pSetQueue)
	{
