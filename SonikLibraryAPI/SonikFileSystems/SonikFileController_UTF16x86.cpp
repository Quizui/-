...
namespace SonikLibFileSystemsControllers
{
	...
	bool SonikFileStreamController_UTF16MODE::ReadText_CHR(int8_t* buffer, uint32_t ReadSize)
	{
		atmlock.lock();
		if( !Opend )
		{
	...
	bool SonikFileStreamController_UTF16MODE::ReadText_Line(SonikLib::SonikString& LineStr, uint32_t GetRowCnt)
	{
		atmlock.lock();
		if( !Opend )
		{
	...
	bool SonikFileStreamController_UTF16MODE::ReadText_LineQueue(SonikQueue::SonikAtomicQueue<SonikLib::SonikString>& LineStrQueue, uint32_t GetRowCnt)
	{
		atmlock.lock();
		if( !Opend )
		{
