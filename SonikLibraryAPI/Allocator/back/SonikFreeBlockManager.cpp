...
{
	//カテゴリ算出
	void GetCategory(uint32_t Val, uint32_t& First, uint32_t& Second, SonikTLSFSecondCategoryBitDepth SetDepth)
	{
		First = Val;
		Second = Val;
...
namespace SonikBTMAFreeBlock
{
	...
	//イニシャライザ
	bool FreeBlockManager::Initialize(uint32_t SetSize, SonikTLSFSecondCategoryBitDepth SetSecondBitDepth)
	{
		uint32_t Bit = SonikMathBit::GetMSBFor32bit(SetSize);
		...
		for(uint32_t looped_ =0; looped_ < Bit; ++looped_)
		{
	...
	{
		uint32_t pValFirst =0;
		uint32_t pValSecondary = 0;
	...
	//フリーブロックを取得します。
	SonikBaundaryBlock::SonikBaundaryPre* FreeBlockManager::GetFreeBlock(uint32_t GetSize)
	{
		uint32_t pValFirst = GetSize;
		uint32_t pValSecondary =  GetSize;
		uint32_t msbcount = 0;
	...
	{
		uint32_t pValFirst; 		//ファーストカテゴリ値
		uint32_t pValSecondary;	//セカンダリカテゴリ値
