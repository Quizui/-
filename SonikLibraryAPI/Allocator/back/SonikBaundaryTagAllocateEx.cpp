...
namespace SonikBTMA
{
	...
	public:
		...
		//コンストラクタオーバーロード
		AllocImpl(uint32_t SetAllocSize, SonikTLSFSecondCategoryBitDepth SetDepth = STLSF_Depth16)
		...
		//メモリの取得
		SonikBaundaryPre* GetAlloc(uint32_t AllocSize)
		{
			if(AllocSize == 0)
			{
				return 0;
			...
			//フリーリストのチェック==========
			uint32_t tmpsize = AllocSize + (sizeof(SonikBaundaryPre) + sizeof(SonikBaundaryPost));
			SonikBaundaryPre* PrePtr = 0;
...
//					Flags |= 0x01;
			...
			M_TopPrePtr->UseUserMemorySize = ( reinterpret_cast<uintptr_t>(M_EndPostPtr) - reinterpret_cast<uintptr_t>(pTmp) );
	...
	//初期化
	bool SonikBaundaryTagAllocateEx::Initialize(uint32_t SetAllocateSize, SonikTLSFSecondCategoryBitDepth SetDepth)
	{
		try
		{
	...
	//取得
	bool SonikBaundaryTagAllocateEx::GetAlloc(void*& RefGetAllocPtr, std::atomic<uint32_t>*& RefGetRefCounter, uint32_t GetAllocSize, uint32_t ArrayCount)
	{
