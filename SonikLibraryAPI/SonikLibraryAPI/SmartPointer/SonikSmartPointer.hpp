...
namespace SonikLib
{
	...
	template <class pType, class Enable> //リテラル型がデフォルト
	class SharedSmtPtr
	{
	...
	//クラス型用の特殊化。
	template <class pType>
	class SharedSmtPtr<pType, typename std::enable_if<std::is_class<pType>::value>::type>
	{
		static_assert(!std::is_pointer_v<pType>, "Please Used NoPointerType");
	...
	//クラス用特殊化
	template <class pType>
	class UniqueSmtPtr<pType, typename std::enable_if<std::is_class<pType>::value>::type>
	{
		static_assert(!std::is_pointer_v<pType>, "Please Used NoPointerType");
